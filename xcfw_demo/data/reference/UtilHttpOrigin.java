...
public class UtilHttpOrigin {
    ...
    public static HttpURLConnection requestByGet(String urlStr,
        ...
        if (requestParams == null || requestParams.isEmpty()) {
            ...
            for (Map.Entry<String, Object> entry : requestParams.entrySet()) {
                ...
                value = URLEncoder.encode(value, XCConstant.ENCODING_UTF8);
    ...
    public static HttpURLConnection requestByPost(String urlStr,
        ...
        conn.setRequestMethod(XCConstant.POST);
        conn.setConnectTimeout(10000);
    ...
    public static InputStream httpClientByPost(String url,
        ...
        UrlEncodedFormEntity form = new UrlEncodedFormEntity(param, XCConstant.ENCODING_UTF8);
        post.setEntity(form);
    ...
    public static String httpClientUpload(String url, String[] names, File file) throws ClientProtocolException, IOException {
        ...
        form.addPart("filename", new StringBody(names[0], Charset.forName(XCConstant.ENCODING_UTF8)));// 文件名
        form.addPart("filedes", new StringBody(names[1], Charset.forName(XCConstant.ENCODING_UTF8))); // 文件描述
        form.addPart("formfile", new FileBody(file)); // 文件
        post.setEntity(form);
        HttpResponse response = client.execute(post);
        if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
            InputStream in = response.getEntity().getContent();
            return new String(XCIO.toBytesByInputStream(in), XCConstant.ENCODING_UTF8);
        }
        return null;
    }
}
