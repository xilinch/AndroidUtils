...
public class SXPickerView extends View {
    ...
    public void setSelected(String mSelectItem) {
        for (int i = 0; i < mDataList.size(); i++)
            if (mDataList.get(i).getContent().equals(mSelectItem)) {
                setSelected(i);
                break;
            }
    }
    ...
    private void drawData(Canvas canvas) {
        ...
        canvas.drawText(mDataList.get(mCurrentSelected).getContent(), x, baseline, mPaint);
        // 绘制上方data
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        FontMetricsInt fmi = mPaint.getFontMetricsInt();
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        canvas.drawText(mDataList.get(mCurrentSelected + type * position).getContent(),
